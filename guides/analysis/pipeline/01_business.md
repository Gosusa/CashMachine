...
**확정 강점**:
- {입증된 해자(established), 활성 이익 경로, 차별화 우위 중 핵심 1~3}

**확정 취약점**:
- {unverified 해자, 해자 훼손 징후, 경쟁 위협 중 모니터링 필요한 1~2}

**다음 모듈 핸드오프**:
- 성장 동력 후보 (→ 모듈 2): {수요 동력 + 전략 베팅에서 도출 1~3}
  ...
  segments: [
    {
      ...
      strategic_bet: { what, why, aiming_for, differentiation_vs_competitors },
...
4가지를 모두 채움. 근거는 **결정과 결과** (capex 추이·R&D 추이·M&A 이력·인력 채용 패턴·세그먼트 자원 배분)에서 추출. 발언이 아닌 자원 흐름이 베팅이다.
...
- **확정 강점** — (b)~(f)에서 입증된 항목 중 핵심 1~3 (예: established 해자, 활성 이익 경로, 차별화 우위)
- **확정 취약점** — (d) unverified·훼손, (e) 경쟁 위협 중 모니터링이 필요한 1~2
- **다음 모듈 핸드오프**:
  - 성장 동력 후보 (→ 모듈 2): (c) 수요 동력 + (f) 전략 베팅에서 도출 1~3
...
- [ ] (f) 전략적 베팅 4개 항목(무엇·왜·무엇을 얻고자·경쟁사 차별화) 모두 채워짐
