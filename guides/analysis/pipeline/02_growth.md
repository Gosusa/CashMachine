...
> {회사 한 줄 요약 + 두 판정 한 줄 (15%+ 복리 / 해자 평생 보유)}
...
- **회사 전체 해자 평생 보유 가능성**: YES/조건부/NO + 근거 한 줄
...
```
{
  ...
  company_overall: {
    aggregate_15pct_feasibility,            // 'YES'|'CONDITIONAL'|'NO' + 근거
    moat_lifelong_holdability,              // 'YES'|'CONDITIONAL'|'NO' + 근거
...
- **회사 전체 15%+ 가능성** — 사업부별 결론(`fifteen_pct_feasibility`)을 매출 비중에 비추어 종합 (사업부별 결론 함께 제시)
- **회사 전체 해자 평생 보유 가능성** — 사업부별 해자(M1 `moats`·`moat_trend`·`moat_erosion`)와 변화 후 추세(`moat_trend_after_change`)를 핵심 사업부(가치 무게중심) 기준으로 종합. YES/조건부/NO + 근거. 핵심 사업부 해자 약화·훼손 임박 시 NO 또는 조건부
- **종합 성장 성격** — 복리형/사이클형/전환 중/혼합 (한두 줄)
- **회사 차원 핵심 동력** — 여러 사업부에 공통 적용되는 동력
- **회사 차원 핵심 리스크** — 복수 사업부에 동시 영향 주는 리스크

회사 종합은 요약일 뿐, 사업부별 결론을 덮어쓰지 않는다. 두 판정(15%+ / 해자 평생 보유)은 모듈 5 리포트의 TL;DR에 직접 매핑된다.
